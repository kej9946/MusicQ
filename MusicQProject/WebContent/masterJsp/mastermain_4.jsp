	...
	<main>
		...
		<section>
			...
			<table class="admin-table">
				...
				<%
					// 문의사항 리스트 가져오기    
				List<QnaVO> qList = (List<QnaVO>) request.getAttribute("qnaList");
				// 문의사항 리스트가 존재하는 경우에만 출력
				if (qList != null && !qList.isEmpty()) {
				
					Collections.sort(qList, new Comparator<QnaVO>() {
						@Override
					...
					for (QnaVO munE : qList) {
				%>
				<tr>
					<td><a
						href="/MusicQProject/detail_munE?qTitle=<%=munE.getQnaTitle()%>"><%=munE.getQnaNo()%></a></td>
					<td><%=munE.getQnaTitle()%></td>
					<td><%=munE.getQnaDt()%></td>
					<td>
						<form action="/MusicQProject/delete_munE" method="post"
							style="display: inline;" onsubmit="return confirmDelete();">
							<input type="hidden" name="qnaTitle"
								value="<%=munE.getQnaTitle()%>">
			...
			<script>
				function confirmDelete() {
					return confirm("정말로 해당 문의사항을 삭제하시겠습니까?");
				}
			</script>
